...
if exist "D:\db_backups" goto BACKUP
...
if exist "D:\db_backups\%DUMPFILENAME%.gz" (
  echo Recent backup exists
  echo Renaming file ...
  ren "D:\db_backups\%DUMPFILENAME%.gz" "%DUMPFILENAME%-old.gz"
  echo.
)
mongodump -h 127.0.0.1:27017 -d admin --archive="D:\db_backups\%DUMPFILENAME%.gz" --gzip
if %errorlevel% EQU 0 (
