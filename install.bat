...
:COPYAPPFILES
if %OS%==x86 (
  if exist "%~dp0x86\bundle\" if exist "%~dp0x86\bundle.tar" if exist "%~dp0x86\bundle.tar.xz" if exist "%~dp0x86\bundle.7z" (
    goto RENFILES
  ) else goto EXTRACTFILES
)
if %OS%==x64 (
  if exist "%~dp0x64\bundle\" if exist "%~dp0x64\bundle.tar" if exist "%~dp0x64\bundle.tar.xz" if exist "%~dp0x64\bundle.7z" (
    goto RENFILES
  ) else goto EXTRACTFILES
)
...
:EXTRACTFILES
if exist "%INSTALLDIR%\bundle.tar.xz" if exist "%INSTALLDIR%\bundle.tar" if exist "%INSTALLDIR%\bundle.7z" (
  if %UNATTENDED% EQU 0 (
  ...
  if %OS%==x86 (
    set ARCH=os.windows.x86_32
  ) else set ARCH=os.windows.x86_64
  meteor build --server-only --directory %INSTALLDIR% --architecture %ARCH%
)
...
echo.
REG ADD "HKLM\SOFTWARE\%APPTITLE%\%APPNAME%" /v "InstallDir" /t REG_SZ /d %INSTALLDIR% /f
REG ADD "HKLM\SOFTWARE\%APPTITLE%\%APPNAME%" /v "Version" /t REG_SZ /d %APPVER% /f
REG ADD "HKLM\SOFTWARE\%APPTITLE%\%APPNAME%" /v "Helper" /t REG_DWORD /d %HELPER% /f
echo.
